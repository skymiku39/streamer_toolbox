...
class ChatSession:
    ...
    async def handle_message(self, data: dict[str, Any]) -> None:
        ...
        if action == "start":
            channel = str(data.get("channel") or data.get("video") or "").strip()
            if not channel:
                await self.send(
                    {"type": "status", "status": "error", "message": "缺少 channel 參數"}
                )
                return
            await self.start_reading(channel)
        elif action == "stop":
    ...
    async def start_reading(self, channel: str) -> None:
        await self.stop_reading()
        await self.send(
            {
                "type": "status",
                "status": "connecting",
                "message": f"正在連線 #{channel} ...",
            }
        )
        ...
        try:
            self.reader = LiveChatReader(channel)
        except ValueError as exc:
            await self.send(
                {
                    "type": "status",
                    "status": "error",
                    "message": f"頻道名稱無效: {exc}",
                }
            )
            return
        ...
        def _run_reader() -> None:
            ...
            try:
                for message in self.reader.iter_messages():
...
def main(argv: list[str] | None = None) -> int:
    _ensure_utf8_stdout()
    parser = argparse.ArgumentParser(description="TTVChat Lens WebSocket 伺服器")
    parser.add_argument("--host", default=DEFAULT_HOST, help="綁定 host (預設 127.0.0.1)")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help="綁定 port (預設 8766)")
    parser.add_argument(
        "--log-level",
        default="INFO",
