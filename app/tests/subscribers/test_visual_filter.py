...
def test_accepts_normal_message() -> None:
    filt = SubtitleMessageFilter(FilterConfig())
...
def test_blocks_short_messages() -> None:
    filt = SubtitleMessageFilter(FilterConfig(min_length=5))
...
def test_blocks_keywords_case_insensitive() -> None:
    filt = SubtitleMessageFilter(FilterConfig(blocked_keywords=["spam"]))
...
def test_blocks_urls_when_enabled() -> None:
    filt = SubtitleMessageFilter(FilterConfig(block_urls=True))
