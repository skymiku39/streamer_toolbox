...
def test_create_knowledge_store_db_only_uses_chroma_rag(monkeypatch, tmp_path: Path) -> None:
    ...
    assert isinstance(store, ChromaSummaryKnowledgeStore)


def test_create_knowledge_store_static_only_uses_chroma_rag(
    monkeypatch,
    tmp_path: Path,
) -> None:
...
def test_create_knowledge_store_rejects_file_backend(monkeypatch, tmp_path: Path) -> None:
    knowledge_file = tmp_path / "notes.txt"
    knowledge_file.write_text("x", encoding="utf-8")
    monkeypatch.setenv("LLM_MEMORY_FROM_DB", "false")
    monkeypatch.setenv("LLM_KNOWLEDGE_BACKEND", "file")
    with pytest.raises(ValueError, match="LLM_KNOWLEDGE_BACKEND=file"):
