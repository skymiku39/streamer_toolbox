...
async def run(channel: str, config: LocalAudioConfig) -> None:
    ...
    capture = MicAudioCapture(
        chunk_seconds=stt_config.chunk_seconds,
    ...
    capture.start()
    device_label = config.device if config.device is not None else "default"
    print(
        f"Capturing local mic (device={device_label}) for #{normalized_channel} → stt.segment",
        file=sys.stderr,
        flush=True,
    )
...
@register_publisher(
    name="ingress-local-audio",
    exchange=DEFAULT_EXCHANGE,
    description="Local mic audio STT → RabbitMQ stt.segment",
)
def main(argv: list[str] | None = None) -> int:
    load_dotenv()
    parser = argparse.ArgumentParser(
        description="Local mic audio STT → RabbitMQ stt.segment publisher",
    )
