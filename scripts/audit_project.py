...
NAMING_AUDIT_MODULE = ROOT / "scripts" / "naming_audit.py"
...
def _load_naming_audit() -> ModuleType:
    spec = importlib.util.spec_from_file_location("naming_audit", NAMING_AUDIT_MODULE)
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    sys.modules["naming_audit"] = module
    spec.loader.exec_module(module)
    return module


_NAMING_AUDIT = _load_naming_audit()
INTENTIONAL_PARALLEL_CLASSES = _NAMING_AUDIT.INTENTIONAL_PARALLEL_CLASSES
...
# --- 命名風險（規則五，見 scripts/naming_audit.py）------------------------


def _naming_check(name: str, checker, root: Path) -> CheckResult:
    ok, detail, hint = checker(root)
...
def check_cross_package_duplicate_class(root: Path = ROOT) -> CheckResult:
    return _naming_check(
        "naming_cross_package_class",
        _NAMING_AUDIT.check_cross_package_duplicate_class,
        root,
    )


def check_app_package_duplicate_class(root: Path = ROOT) -> CheckResult:
    return _naming_check(
        "naming_app_package_class",
        _NAMING_AUDIT.check_app_package_duplicate_class,
        root,
    )


def check_cross_package_duplicate_function(root: Path = ROOT) -> CheckResult:
    return _naming_check(
        "naming_cross_package_function",
        _NAMING_AUDIT.check_cross_package_duplicate_function,
        root,
    )
