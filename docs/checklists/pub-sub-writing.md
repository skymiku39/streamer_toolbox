...
L1 記錄層：`chat.message` → SQLite `text_records`。實作於 **`app/subscribers/stream_record.py`**（`@register_subscriber` 自動註冊）。
...
L2 記憶層：定時讀未摘要 chat → 寫 `summaries` 表。實作於 **`app/workers/`**（非 MQ Subscriber）。
