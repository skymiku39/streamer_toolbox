...
| `sub-stream-record` | `llm_twitchat` | `services/stream_session.py`（概念） |
| `sub-memory-worker` | `llm_twitchat` | `memory/knowledge.py`, `services/stream_session.py` |
| `pkg-stream-store` | — | 本專案 `pkg-stream-store/` |
...
### `sub-stream-record` ✅（Phase 1：聊天室）

L1 記錄層：`chat.message` → SQLite `text_records`。

| 項目 | 內容 |
|------|------|
| 訂閱 | `chat.message` |
| 發布 | — |
| 依賴 | `pkg-events`, `pkg-bus`, `pkg-stream-store` |
| 設定 | `STREAM_DB_PATH`, `STREAM_SESSION_ID`, `RECORD_MODE=chat` |
| 架構 | [stream-memory-pipeline.md](../architecture/stream-memory-pipeline.md) |

**撰寫清單**

- [x] subscribe `chat.message` → `StreamTextStore.append_chat`
- [x] `RECORD_MODE=chat` 驗證（Phase 1 僅聊天室）
- [x] 自動 session id：`{channel}_{YYYYMMDD}` 或 `STREAM_SESSION_ID`
- [x] 註冊 `app.main run sub-stream-record`
- [x] 單元測試：`ChatRecordWriter`

**驗收：** 與 `ingress-ttv-read` 同跑時，`text_records` 有新增列。

---

### `sub-memory-worker` ✅（Phase 1）

L2 記憶層：定時讀未摘要 chat → 寫 `summaries` 表。
...
| 發布 | — |
| 依賴 | `pkg-stream-store` |
| 設定 | `STREAM_DB_PATH`, `MEMORY_INTERVAL_MINUTES`, `MEMORY_LLM_BACKEND` |
| CLI | `uv run sub-memory-worker` / `--once` |

**撰寫清單**

- [x] `fetch_unsummarized_chat` → 摘要 → `save_summary` → `mark_summarized`
- [x] `MEMORY_LLM_BACKEND=template` 規則摘要（無 API key）
- [x] 可選 `openai` / `gemini` LLM 摘要
- [x] 單元測試：`MemoryWorker.run_once`

**驗收：** `--once` 後 `summaries` 有列且 `text_records.summarized=1`。
