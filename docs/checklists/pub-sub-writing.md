...
| `sub-stream-record` | `llm_twitchat` | `app/subscribers/sub_stream_record/` |
...
Phase 01 Twitch IRC 匿名讀取。**現況無獨立程序**：功能已併入下方的 `ingress-ttv-read`（包 `packages/ttvchat-lens`），請以該節與 [modules.md](../modules.md) 為準。下表保留 Phase 01 歷史紀錄。
...
- [x] ~~註冊 `app.main run ingress-twitch-chat`~~（已併入 `ingress-ttv-read`，無獨立 process）
...
### `ingress-local-audio` ✅（開發用）

主播本機麥克風 STT，用於改善 chat ↔ 語音時間對齊（詳見 [stream-memory-pipeline.md](../architecture/stream-memory-pipeline.md#替代方案主播本機擷取音訊)）。**現行實作為麥克風擷取**；loopback／混音仍為規劃項。

| 項目 | 內容 |
|------|------|
| 發布 | `stt.segment` |
| 依賴 | `stt-core`, `safety`；本機麥克風（`sounddevice`） |
| 部署 | **僅主播開台 PC**；與 `ingress-ttv-read` 同機 |
| 路徑 | `app/publishers/ingress_local_audio/` |
| 設定 | `STT_*`（共用 `SttConfig.from_env()`）、`LOCAL_AUDIO_DEVICE` |

**撰寫清單**

- [x] 本機麥克風 PCM 擷取 → chunk
- [x] 重用 `stt_core.StreamingSTTWorker` + `build_stt_segment_event` 契約
- [x] `@register_publisher` → `ingress-local-audio`（`--stack ingress`）
- [x] 環境變數：`LOCAL_AUDIO_DEVICE`（裝置 index 或名稱）
- [ ] loopback／混音擷取（Future）
- [ ] 單元測試：mock PCM chunk → segment JSON

**驗收：** 本機 IRC + 本機 STT 同跑 → `sub-stream-record`（`RECORD_MODE=both`）收到 `stt.segment`；chat/stt timestamp 差值主要反映主播反應時間。
...
L1 記錄層：`chat.message` / `stt.segment` → SQLite `text_records`。實作於 **`app/subscribers/sub_stream_record/`**。
...
| 路徑 | `app/subscribers/sub_stream_record/` |
...
直播狀態宣告 Sub：收到 `stream.metadata`（開播）後發布一次 `chat.reply` 問候／狀態訊息（**不含** `!ask` 問答）。

| 項目 | 內容 |
|------|------|
| 訂閱 | `stream.metadata` |
| 發布 | `chat.reply`（`source: logic-status`） |
| 依賴 | `events`, `bus`, `stream-store`（冪等去重） |
| 路徑 | `app/subscribers/sub_live_status/` |
| 設定 | `LIVE_STATUS_ANNOUNCE`（是否發布）、`TWITCH_CHANNEL` |

**撰寫清單**

- [x] 訂閱 `stream.metadata`，解析 `StreamMetadataEvent`
- [x] 開播時發布狀態宣告至 `chat.reply`（經 `twitch-connector` 發話）
- [x] SQLite 冪等：同一場次僅宣告一次（`sub_live_status.startup` namespace）
- [x] 不處理 `!ask`、不呼叫 LLM
- [ ] 單元測試：mock metadata → 預期 `chat.reply` payload

**驗收：** `--stack ingress` + `--stack llm` 開播後，聊天室收到狀態宣告且不重複刷屏。
