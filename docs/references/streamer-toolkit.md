...
**不是**最終 `stream-core` 形態：訊息 schema、MQ 拓撲與 package 拆分尚未完全對齊 [events.md](../events.md) 與 [phase-01 計畫](../plans/phase-01-rabbitmq-io-poc.md)。
...
| `pkg-events` | `ChatMessage` schema | `app/messaging/schemas.py` |
| `pkg-bus` | RabbitMQ helpers | `app/messaging/rabbitmq.py` |
| MQ 命名常數 | Exchange / Queue | `app/messaging/topology.py` |
| `stream-app`（Phase 04） | CLI + ProcessRegistry + Runner | `app/main.py`、`app/processes/` |
...
| **D** | Sub 依賴 toolkit 內 `ChatMessage`，非獨立 `pkg-events` |
...
1. **Schema 對齊** — `ChatMessage` 擴充為 `events.md` 完整 `chat.message` payload
2. **拓撲對齊** — fanout `twitch.chat` → topic `stream_helper` + routing key `chat.message`
3. **抽出 pkg** — `schemas.py` → `pkg-events`；`rabbitmq.py` → `pkg-bus`（`EventBus` Protocol）
4. **命名對齊** — `pub1`/`sub1` → `ingress-twitch-chat` / `sub-io-log`；`sub1` 改 JSONL 輸出
5. **編排演進** — 現有 CLI runner 可過渡為 Phase 04 `stream-app` 的雛形
